...
perl -pe 's/^([^\t]+)_(\d+)\t/TID_$2\t/' aggregated_counts.tsv >rdp_otu_table
perl -ne 'if(/^([^\t]+)_(\d+)\t/){print "TID_$2\t"; $tax=$1; $tax=~s/_\d+,/\t/g; $tax=~s/__sub__/__/g; $tax=~s/__super__/__/g; print "$tax\n"; }' aggregated_counts.tsv >rdp_tax_table
#+END_SRC
The files
